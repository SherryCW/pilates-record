...
const extraExercisesWithCustomImages: Exercise[] = extraExercises.map(exercise => {
  ...
  if (exercise.kind === 'Reformer' && exercise.en === 'Frog') return { ...exercise, image: assetUrl('assets/reformer/reformer-frog.png?v=2'), sprite: undefined, tileX: undefined, tileY: undefined }
...
const customMoreExercises: Exercise[] = moreExercises.map(exercise => exercise.en === 'Small Ball Leg Lift' ? { ...exercise, image: assetUrl('assets/small-apparatus/small-ball-leg-lift.png?v=1'), sprite: undefined, tileX: undefined, tileY: undefined } : exercise)
...
const reformerCategoryNames: Record<Exclude<ReformerCategory, '全部'>, string[]> = {
  '脚踏板与仰卧': ['Footwork', 'The Hundred', 'Frog', 'Leg Circles', 'Short Spine', 'Footwork Toes', 'Footwork Heels', 'Long Spine Massage', 'High Frog', 'Semi Circle', 'High Bridge', 'Pelvic Lift', 'Single Leg Footwork', 'Footwork on Footplate', 'Jumping on Footplate', 'Supine Arm Work', 'Bridging', 'Feet in Straps', 'Reverse Abdominals', 'Running', 'Coordination'],
  '长箱': ['Long Box Pulling Straps', 'Backstroke', 'Swan on Long Box', 'Breaststroke', 'Hamstring Curls', 'Horseback', 'Side Sit Ups', 'Pulling Straps', 'T-Pull', 'Grasshopper', 'Swimming', 'Long Box Double Leg Kick', 'Rocking'],
  ...
  '坐姿与划船': ['Rowing Into the Sternum', 'Rowing 90 Degrees', 'Rowing From the Chest', 'Rowing From the Hips', 'Rowing Back', 'Rowing Front', 'Shaving', 'Hug', 'Teaser', 'Mermaid', 'Stomach Massage', 'Stomach Massage Round', 'Stomach Massage Hands Back', 'Stomach Massage Reach Up', 'Stomach Massage Twist', 'Side Stretch / Mermaid', 'Cleopatra', 'Biceps Curl', 'Posterior Shoulder Press', 'Serve a Tray'],
  '站姿与侧向': ['Single Leg Elephant', 'Arabesque', 'Front Splits', 'Russian Splits', 'Side Splits', 'Tendon Stretch', 'Tendon Stretch Side', 'Lunges', 'Scooter', 'Side Support'],
...
export default function App() {
  ...
  return <main className="fitness-app">
    ...
    {step === 'edit' && <section className="sheet edit-sheet"><div className="section-heading"><div><span className="eyebrow">Training Log</span><h2>填写今天的训练</h2></div><button className="text-button" onClick={() => setStep('choose')}>← 返回选动作</button></div><div className="edit-layout"><div className="edit-list">{chosen.map(exercise => <article className="edit-row" key={exercise.id}>{exercise.sprite ? <div className="edit-sprite" role="img" aria-label={exercise.en} style={spriteStyle(exercise)} /> : <div className="edit-image-frame"><img className={exerciseImageClass(exercise)} src={exercise.image} alt="" /></div>}<div className="edit-main"><div className="edit-row-heading"><div className="edit-title"><h3>{exercise.zh}</h3><small>{exercise.en}</small></div><button className="remove-exercise" type="button" aria-label={`删除动作：${exercise.zh} ${exercise.en}`} onClick={() => removeExercise(exercise.id)}>删除动作</button></div>{(logs[exercise.id] || [{ weight: '', reps: '' }]).map((set, index) => <div className="set-line" key={index}><span>第 {index + 1} 组</span><input inputMode="decimal" placeholder="重量" value={set.weight} onChange={e => updateSet(exercise.id, index, 'weight', e.target.value)} /><b>kg ×</b><input inputMode="numeric" placeholder="次数" value={set.reps} onChange={e => updateSet(exercise.id, index, 'reps', e.target.value)} /><b>次</b></div>)}<button className="add-set" onClick={() => addSet(exercise.id)}>＋ 添加一组</button></div></article>)}{!chosen.length && <div className="empty-training"><span className="eyebrow">No Exercise Selected</span><h3>还没有训练动作</h3><p>返回动作库重新选择，已删除动作的训练数据不会保留。</p><button className="secondary" type="button" onClick={() => setStep('choose')}>返回选择动作</button></div>}</div><MuscleMap chosen={chosen} /></div><div className="action-bar"><span>{chosen.length ? `${chosen.length} 个动作 · 数据只保留在当前页面` : '请至少选择 1 个动作'}</span><button className="primary" disabled={!chosen.length} onClick={makeShare}>确认训练 · 生成分享图</button></div></section>}
